...
public record AuthenticationRoute(
    ...
    private Optional<APIGatewayProxyResponseEvent> handleAuthentication(final APIGatewayProxyRequestEvent event) {
        ...
        if (event.getHttpMethod().equalsIgnoreCase("DELETE")) {
            // redirect url
            ...
            response.setStatusCode(200);
            HashMap<String, String> headers = new HashMap<>();
            headers.put("Set-Cookie", "session=;max-age=0");
            headers.put("Content-Type", "text/html");
            headers.put("Cache-control", "private, no-cache, no-store, max-age=0, must-revalidate");
            response.setHeaders(headers);
            response.setBody("<!DOCTYPE html><html><body><script type='text/javascript'>window.location = '"
                    + redirectUrl + "';</script></body></html>");
            return Optional.of(response);
        }
        ...
        if (code == null || scope == null || code.isBlank() || scope.isBlank()) {
            this.redirectToGoogleAuth(event);
            return Optional.empty();
        }
