...
public record ShipSheetRoute(TemplateService templateService) implements AuthenticatedRequestHandler {
    ...
    public APIGatewayProxyResponseEvent handleRequest(
        ...
        final String shipPathParameter = event.getPathParameters().get("ship");
        if (shipPathParameter == null || shipPathParameter.isBlank()) {
            return null;
        }

        final ShipIdentifier shipIdentifier = new ShipIdentifier(shipPathParameter);
        final Ship ship = this.templateService.shipService().retrieve(shipIdentifier);
        if (ship == null) {
            return null;
        }
        ...
        ctx.put("ship", new ShipSchema(ship));
        ctx.put("url", urlResolver);

        Optional.ofNullable(ship.campaignIdentifier())
        ...
        final String html = templateService.parse("ship-sheet", ctx);
